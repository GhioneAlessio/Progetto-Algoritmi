...
static  void free_array(BinaryInsertionSort* array) {
  unsigned long  el_num =  binary_insertion_sort_size(array);
  for(unsigned long i=0;i<el_num;i++){
    struct record *array_element = (struct record *)binary_insertion_sort_get(array, i);
    free(array_element->string_field);
    free(array_element);
  }
  binary_insertion_sort_free_memory(array);
}

static  void print_array(BinaryInsertionSort* array){
  unsigned long el_num =  binary_insertion_sort_size(array);
  ...
  for(unsigned long i=0;i<el_num;i++){
    array_element = (struct record *)binary_insertion_sort_get(array, i);
    printf("<%s,%d>\n",array_element->string_field,array_element->integer_field); 
  }
}

static void load_array(const char* file_name, BinaryInsertionSort* array){
  char *read_line_p;
  char buffer[1024];
  int buf_size = 1024;
  FILE *fp;
  ...
  while(fgets(buffer,buf_size,fp) != NULL){  
    ...
    binary_insertion_sort_add(array, (void*)record_p);
    free(read_line_p);
  }
  fclose(fp);
  printf("\nData loaded\n");
...
static  void test_with_comparison_function(const char* file_name, int (*compare)(void*, void*)) {
  BinaryInsertionSort* array = binary_insertion_sort_create(compare);
...
int main(int argc, char const *argv[]) {
  if(argc < 2) {
    printf("Usage: binary_insertion_sort_main <file_name>\n");
    exit(EXIT_FAILURE);
  }
